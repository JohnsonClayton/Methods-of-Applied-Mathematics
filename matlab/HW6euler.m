function euler211(x0,y0,h,N)
%Euler's Method from Chapter 21.1 (hence euler211).  
...
%Euler's Method
for n = 1:N+1;
  x(n+1) = x(n)+h;              %x(n+1)= next x-value using step size h
  y(n+1) = y(n)+h*f(x(n),y(n)); %f(x(n),y(n))= slope at current point
...
xlabel('x')
ylabel('y')
k = legend('Euler Method','Analytic Solution', 'Location','best');
set(k,'fontsize',12);
