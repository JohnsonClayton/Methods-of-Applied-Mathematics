"""
This program implements the Euler Method for solving 
the IVP y' = f(x,y), y(x0) = y0.
This method comes from Ch21.1, and hence "euler211".  
...
def euler211(x0,y0,h,N):  #This defines the program as a function. 
    ...
    for n in range(0,N): #This loop computes the numerical solution values
        x[n+1] = x[n] + h  #next x-value using step size h
        y[n+1] = y[n] + h*f(x[n],y[n])  #next y value from Euler's method
    
    print('Euler Solution=', y)     #Print numerical solution y-values
