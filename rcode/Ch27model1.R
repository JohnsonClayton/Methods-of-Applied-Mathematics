Ch27model1 <- function(T,n) {
  ...
  #System Parameters
  t0 <- 0
  xd0 <- 1         #initial value for GI decongestant
  xa0 <- 1         #initial value for GI antihistamine
  ...
  for(i in 1:n) {
    a1 <- h*f2d(xd[i],yd[i]);          #f1= slope of xd
    a2 <- h*f2a(xa[i],ya[i]);          #f2= slope of xa
    ...
    yd[i+1] <- yd[i]+(1/6)*(a1+2*b1+2*c1+d1); #Next yd value 
    ya[i+1] <- ya[i]+(1/6)*(a2+2*b2+2*c2+d2); #Next ya value 
    }
  ...
  legend("topright",
         legend = c("Decongestant", "Antihistamine"),
         col=c("blue","red"),  #legend colors
         lty=c(1,1.5)  #lty = line type (for legend colors)
         )
